...
export async function fetchLatestInvoices() {
  try {
    const data = await sql<LatestInvoiceRaw>`
      SELECT invoices.amount, customers.name, customers.image_url, customers.phone, invoices.id
      FROM invoices
...
export async function fetchFilteredCustomers(query: string) {
  try {
    const data = await sql<CustomersTableType>`
		...
		GROUP BY customers.id, customers.name, customers.phone, customers.image_url
